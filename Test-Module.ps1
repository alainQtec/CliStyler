...
process {
    ...
    $ntTestsPath = $testFiles.Where({ $_.BaseName -eq 'CliStyler.Intergration.Tests' }).FullName
    $ftTestsPath = $testFiles.Where({ $_.BaseName -eq 'CliStyler.Features.Tests' }).FullName
    $mtTestsPath = $testFiles.Where({ $_.BaseName -eq 'CliStyler.Module.Tests' }).FullName
